...
| E2E-20260303-007 | P2 | WAIT_USER | phase3-e2e-008 | 移动端导航抽屉存在无可访问名称的按钮 | 移动端打开菜单（顶部汉堡按钮），观察控制台 axe 输出 | 交互按钮应具备可访问名称，不触发 serious a11y 错误 | 控制台出现 `[serious] control-has-accessible-name at <button>`，来源于菜单弹层按钮 | `logs/e2e-mcp/20260303-round8-mobile-critical-path/mobile-browse-menu-open.png`, `logs/e2e-mcp/20260303-round8-mobile-critical-path/console-errors.log`, `logs/e2e-mcp/20260303-round8-mobile-critical-path/round8-report.json` | - | 功能可用但影响可访问性质量门禁 |
