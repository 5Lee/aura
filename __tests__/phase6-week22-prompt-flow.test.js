...
test("prompt flow dashboard exposes visual editing and run log workflow", () => {
  ...
  assert.match(promptFlowPanelSource, /buildFlowForm/)
  assert.match(promptFlowPanelSource, /const selectedFlow = useMemo/)
  assert.match(promptFlowPanelSource, /setFlowForm\(buildFlowForm\(selectedFlow\)\)/)
  ...
  assert.match(promptFlowPanelSource, /DATE_TIME_FORMATTER/)
  assert.match(promptFlowPanelSource, /Asia\/Shanghai/)
  assert.match(promptFlowPanelSource, /formatDateTime\(item\.startedAt\)/)
