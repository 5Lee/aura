...
test("connector dashboard is gated by entitlement and integrated in navigation", () => {
  ...
  assert.match(connectorPanelSource, /buildConnectorForm/)
  assert.match(connectorPanelSource, /formatShanghaiDateTime/)
  assert.match(connectorPanelSource, /timeZone: "Asia\/Shanghai"/)
  assert.match(connectorPanelSource, /const selectedConnector = useMemo/)
  assert.match(connectorPanelSource, /setForm\(buildConnectorForm\(selectedConnector\)\)/)
  ...
  assert.match(navbarSource, /"\/connectors"/)
