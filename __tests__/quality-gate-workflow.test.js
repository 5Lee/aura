...
test("quality gate workflow covers build/typecheck/lint/test on main", () => {
  ...
  assert.match(workflow, /Stage 4 - Prompt Regression/)
  assert.match(workflow, /npm run prompt-regression -- --mode ci --allow-empty/)
  assert.match(workflow, /Stage 5 - Build/)
...
test("quality gate doc includes matrix and branch protection guidance", () => {
  ...
  assert.match(qualityDoc, /Prompt Regression/)
