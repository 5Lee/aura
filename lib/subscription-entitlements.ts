...
const AD_STRATEGY_PLANS = new Set<SubscriptionPlanId>(["pro", "team", "enterprise"])
const PARTNER_PROGRAM_PLANS = new Set<SubscriptionPlanId>(["pro", "team", "enterprise"])
...
export function hasEnterpriseSupportProcessAccess(planId: string) {
  ...
  return ENTERPRISE_SUPPORT_PROCESS_PLANS.has(planId)
}

export function hasMarketplaceCommissionAccess(planId: string) {
  if (!isSubscriptionPlanId(planId)) {
    return false
  }
...
export function hasAdStrategyAccess(planId: string) {
  if (!isSubscriptionPlanId(planId)) {
    return false
  }
  return AD_STRATEGY_PLANS.has(planId)
}

export function hasPartnerProgramAccess(planId: string) {
  if (!isSubscriptionPlanId(planId)) {
    return false
  }
  return PARTNER_PROGRAM_PLANS.has(planId)
}

export function hasGrowthLabAccess(planId: string) {
  if (!isSubscriptionPlanId(planId)) {
    return false
  }
  return GROWTH_LAB_PLANS.has(planId)
}

export function hasConnectorCatalogAccess(planId: string) {
  if (!isSubscriptionPlanId(planId)) {
    return false
  }
  return CONNECTOR_CATALOG_PLANS.has(planId)
}

export function hasPromptFlowAccess(planId: string) {
  if (!isSubscriptionPlanId(planId)) {
    return false
  }
